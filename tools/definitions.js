...
WARNING: This executes a real on-chain transaction. Check DRY_RUN mode.`,
      parameters: {
        ...
        properties: {
          ...
          base_mint: { type: "string", description: "Base token mint address, for your own record-keeping only — not used to prevent duplicate exposure (that check runs automatically on-chain after deploy). Omit if you don't already know it; do not guess." },
...
Screening: minFeeActiveTvlRatio, minTvl, maxTvl, minVolume, minOrganic, minQuoteOrganic, minHolders, minMcap, maxMcap, minBinStep, maxBinStep, timeframe, category, minTokenFeesSol, excludeHighSupplyConcentration, allowedLaunchpads, blockedLaunchpads, entryRsiGateEnabled, entryRsiThreshold, dryRunBypassFilters
Indicators: chartIndicatorsEnabled, indicatorDataSource (meridian|gecko), geckoRequestDelayMs, geckoMaxRetries, geckoResultCacheSec, indicatorEntryPreset, indicatorExitPreset, rsiLength, indicatorIntervals, indicatorCandles, rsiOversold, rsiOverbought, requireAllIntervals
Logging: suspiciousTickLogEnabled
Management: minClaimAmount, outOfRangeBinsToClose, outOfRangeWaitMinutes, oorCooldownTriggerCount, oorCooldownHours, repeatDeployCooldownEnabled, repeatDeployCooldownTriggerCount, repeatDeployCooldownHours, repeatDeployCooldownScope, repeatDeployCooldownMinFeeEarnedPct, minVolumeToRebalance, triggerScreeningWhenIdle, stopLossEnabled, stopLossPct, stopLossRsiConfirm, stopLossRsiThreshold, stopLossRsiInterval, stopLossRsiLength, stopLossRsiMaxWaitMinutes, stopLossRsiRecheckSec, stopLossRsiGuardIntervalMin, stopLossRsiRequireOutOfRange, stopLossRsiResetAfterMinutes, trailingTpRsiConfirm, oorRsiConfirm, lowYieldRsiConfirm, liquidityRsiConfirm, exitRsiThreshold, exitRsiMaxWaitMinutes, exitRsiRecheckSec, exitRsiGuardIntervalMin, deadPositionGuardEnabled, deadPositionMaxSuspiciousMinutes, deadPositionMaxValueUsd, feeStallGuardEnabled, feeStallThresholdHours, feeStallMinAgeMinutes, takeProfitRsiConfirm, takeProfitRsiThreshold, peakRsiExitEnabled, peakRsiExitThreshold, peakRsiExitCooldownSec, peakRsiExitRequireOutOfRange, peakRsiExitMinPnlPctEnabled, peakRsiExitMinPnlPct, takeProfitPct, minSolToOpen, deployAmountSol, gasReserve, positionSizePct
...
Strategy: minBinsBelow, maxBinsBelow, defaultBinsBelow (legacy binsBelow maps to maxBinsBelow), maxDownsideModeEnabled (overrides all bin sizing above — max protocol downside, zero upside), maxDownsideModeFloorPct
