...
/**
 * computeBbWidthPct — Bollinger Band width as a percentage of the middle band.
 * (upperBand - lowerBand) / middleBand * 100. Surfaced as a volatility signal on
 * GMGN-sourced candidates (see tools/gmgn.js condenseGmgnCandidate). Returns null
 * if bands aren't available rather than 0, so callers can distinguish "no data"
 * from "genuinely zero width".
 */
export function computeBbWidthPct(signal) {
  if (!signal) return null;
  const { lowerBand, upperBand, middleBand } = signal;
  if (lowerBand == null || upperBand == null || !middleBand) return null;
...
export async function fetchChartIndicatorsForMint(
  ...
  if (config.indicators.dataSource === "gecko") {
    // refresh has no meaning here — every call already fetches live
    // candles, there's no server-side cache to bust.
    const { getLocalChartIndicators } = await import("./indicators-local.js");
    return getLocalChartIndicators(mint, { interval: normalizedInterval, candles, rsiLength });
  }
  ...
  return agentMeridianJson(`/chart-indicators/${mint}?${search.toString()}`, {
    ...
    // Was making exactly one attempt with no retry at all — confirmed in
    // production logs 2026-08: repeated "Peak RSI exit check failed...
    // Jupiter HTTP 429" from peak-rsi-exit.js (this backend appears to
    // proxy chart data through Jupiter internally, which rate-limits it).
    // agentMeridianJson already has a full retry/backoff mechanism
    // (isRetryableStatus covers 429/502/etc, retryDelayMs respects
    // Retry-After) — it just wasn't being asked to use it here. Kept
    // fairly short (15s budget, 3 attempts) since this function has many
    // callers on tight polling loops (guards checking every ~1min) — the
    // goal is smoothing over a transient rate-limit blip, not turning a
    // real outage into a long hang.
    retry: { maxElapsedMs: 15_000, maxAttempts: 3 },
  });
}
