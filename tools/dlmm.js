...
import { config, computeDeployAmount, MIN_SAFE_BINS_BELOW, MAX_DOWNSIDE_BINS_BELOW } from "../config.js";
...
import {
  ...
  minutesOutOfRange,
  syncOpenPositions,
  reopenMisclosedPositions,
  autoAdoptGhosts,
} from "../state.js";
import { recordPerformance } from "../lessons.js";
import { isBaseMintOnCooldown, isPoolOnCooldown, recordDeployFailure } from "../pool-memory.js";
...
import { registerPostCloseWatch } from "../post-close-tracking.js";
import { appendPnlRecord } from "../pnl-history.js";
...
let _deriveBinArray = null;
let _binIdToBinArrayIndex = null;
let _getBinArrayLowerUpperBinId = null;
let _MAX_BIN_ARRAY_SIZE = null;
...
async function getDLMM() {
  if (!_DLMM) {
    ...
    _deriveBinArray = mod.deriveBinArray;
    _binIdToBinArrayIndex = mod.binIdToBinArrayIndex;
    _getBinArrayLowerUpperBinId = mod.getBinArrayLowerUpperBinId;
    _MAX_BIN_ARRAY_SIZE = mod.MAX_BIN_ARRAY_SIZE;
  }
  return {
    DLMM: _DLMM,
    ...
    deriveBinArray: _deriveBinArray,
    binIdToBinArrayIndex: _binIdToBinArrayIndex,
    getBinArrayLowerUpperBinId: _getBinArrayLowerUpperBinId,
    MAX_BIN_ARRAY_SIZE: _MAX_BIN_ARRAY_SIZE,
  };
}
...
/**
 * How many bins below the active bin are actually usable WITHOUT paying to
 * create a brand-new bin array account — i.e. capped to whatever bin
 * arrays this pool already has initialized (by us or by anyone else who's
 * LP'd there). Walks downward from the active bin's own array, probing
 * each candidate array's on-chain existence via getAccountInfo, and stops
 * at the first one that doesn't exist yet.
 *
 * Why this matters (not just a nice-to-have): each bin array covers a
 * fixed 70 bins (MAX_BIN_PER_ARRAY, verified against the installed SDK's
 * own IDL constants). Extending a position into a bin array nobody has
 * ever touched means WE pay the rent to create that account — for price
 * territory this pool apparently has zero existing liquidity at (that's
 * exactly why nobody's created it). Blindly requesting the protocol
 * maximum (MAX_DOWNSIDE_BINS_BELOW = 1400) regardless of what a pool
 * actually has going on would mean routinely paying that rent for
 * unreachable price ranges.
 *
 * Deliberately conservative: any failure here (RPC hiccup, unexpected SDK
 * shape, etc.) falls back to config.strategy.maxBinsBelow — the existing,
 * already-sane default — rather than guessing large. Bounded to at most
 * ceil(MAX_DOWNSIDE_BINS_BELOW / 70) probes so a pathological pool can't
 * turn this into an unbounded RPC loop.
 */
async function capBinsBelowToExistingBinArrays(pool, activeBinId, requestedBinsBelow) {
  const fallback = Math.min(requestedBinsBelow, Math.max(0, Number(config.strategy.maxBinsBelow) || 0));
  if (requestedBinsBelow <= 0) return 0;

  try {
    // Same functions, same pattern assertRangeDoesNotRequireBinArrayInitialization
    // (right below) already uses to check a range before every deploy — one
    // batched getMultipleAccountsInfo call instead of probing bin arrays one
    // at a time. If this capping still somehow leaves a gap (e.g. the rarer
    // bitmap-extension case that function also checks), that existing check
    // catches it before any transaction is sent — this doesn't need to
    // duplicate that part, just avoid the common case.
    const { getBinArrayIndexesCoverage, getBinArrayKeysCoverage, getBinArrayLowerUpperBinId } = await getDLMM();
    if (!getBinArrayIndexesCoverage || !getBinArrayKeysCoverage) return fallback;

    const poolPubkey = new PublicKey(
      pool.pubkey?.toString?.() || pool.lbPair?.publicKey?.toString?.() || pool.lbPair?.pubkey?.toString?.(),
    );
    const programId = getDlmmProgramId();
    const lower = new BN(activeBinId - requestedBinsBelow);
    const upper = new BN(activeBinId);

    const indexes = getBinArrayIndexesCoverage(lower, upper); // ascending bin-array-index order
    const keys = getBinArrayKeysCoverage(lower, upper, poolPubkey, programId); // same order, parallel array
    const accounts = await getConnection().getMultipleAccountsInfo(keys, "confirmed");

    // Walk backward from the END (the array closest to/containing the
    // active bin) toward the start, counting consecutive EXISTING arrays —
    // stop at the first gap. Bins beyond that gap would require creating a
    // brand-new bin array, which is exactly the rent cost this is meant to
    // avoid paying for price territory nobody's ever LP'd into.
    let confirmedArrays = 0;
    for (let i = accounts.length - 1; i >= 0; i--) {
      if (!accounts[i]) break;
      confirmedArrays++;
    }
    if (confirmedArrays === 0) {
      log("deploy_warn", `Max-downside mode: even the active bin's own bin array doesn't exist yet — falling back to maxBinsBelow (${fallback})`);
      return fallback;
    }

    const cappedLowerIndex = indexes[accounts.length - confirmedArrays];
    const [cappedLowerBinId] = getBinArrayLowerUpperBinId(cappedLowerIndex);
    const usableBinsBelow = Math.max(0, Math.min(requestedBinsBelow, activeBinId - cappedLowerBinId.toNumber()));

    log("deploy", `Max-downside mode: requested ${requestedBinsBelow} bins below, pool has existing bin arrays covering ${usableBinsBelow} of them (${confirmedArrays}/${accounts.length} arrays already initialized) — capping there to avoid paying rent for brand-new bin arrays`);
    return usableBinsBelow;
  } catch (e) {
    log("deploy_warn", `Max-downside mode: couldn't probe existing bin arrays (${e.message}) — falling back to maxBinsBelow (${fallback}) instead of guessing large`);
    return fallback;
  }
}
...
export async function deployPosition({
  ...
  // ── Max-downside mode ──────────────────────────────────────────
  // Alternative to everything above — sits alongside the volatility-
  // interpolated minBinsBelow/maxBinsBelow system and the explicit
  // bins_below/downside_pct params, overriding whichever one the caller
  // used. Enforced here (not just left to the LLM's prompt instructions)
  // so it's guaranteed regardless of what actually got passed in.
  //
  // NOT a blind request for the full protocol maximum (1400 bins) — capped
  // to whatever this specific pool already has existing bin arrays for
  // (capBinsBelowToExistingBinArrays), so this never pays rent to create
  // brand-new bin array accounts reaching into price territory the pool
  // has no existing liquidity at. That's the actual point of "maximize
  // downside": use everything already there, not manufacture more.
  //
  // Also capped at maxDownsideModeFloorPct (default 99.9%) below the
  // active bin's own price — the "top" of the range, since bins_above is
  // forced to 0 here — regardless of how far the existing-bin-array cap
  // above would otherwise allow. Whichever of the two caps is MORE
  // conservative (fewer bins) wins; a pool could technically have bin
  // arrays initialized well past a 99.9% drawdown (e.g. a very mature,
  // heavily-arbitraged pool), but there's no real benefit to sizing a
  // position into territory that extreme just because the accounts happen
  // to exist.
  if (config.strategy.maxDownsideModeEnabled) {
    const existingArraysCap = await capBinsBelowToExistingBinArrays(pool, activeBin.binId, MAX_DOWNSIDE_BINS_BELOW);

    const floorPct = Math.max(0, Math.min(99.999, Number(config.strategy.maxDownsideModeFloorPct ?? 99.9)));
    const floorPrice = activePrice * (1 - floorPct / 100);
    const floorBinId = getBinIdFromPrice(floorPrice, actualBinStep, true);
    const priceFloorCap = Math.max(0, activeBin.binId - floorBinId);

    activeBinsBelow = Math.min(existingArraysCap, priceFloorCap);
    activeBinsAbove = 0;

    if (priceFloorCap < existingArraysCap) {
      log("deploy", `Max-downside mode: ${floorPct}% price floor (${priceFloorCap} bins) is tighter than the existing-bin-array cap (${existingArraysCap} bins) — using the price floor`);
    }
  }
  ...
  // Was dividing by minPrice instead of activePrice — inconsistent with the
  // two calculations right above, which both correctly use activePrice as
  // the reference. For a normal, roughly-symmetric range minPrice stays
  // close to activePrice, so the bug was nearly invisible; max-downside
  // mode's heavily-skewed ranges (minPrice can be a tiny fraction of
  // activePrice) blew it up into 500-7000%+ readings — confirmed in
  // production logs 2026-08: 89.43% downside + 0% upside was showing as
  // "845.95% total" instead of the correct ~89.43% (downside + upside,
  // both measured against the same activePrice reference).
  const totalWidthPct = activePrice > 0 ? ((maxPrice - minPrice) / activePrice) * 100 : null;
  ...
  if (shouldUseLpAgentRelayForDeploy()) {
    try {
      ...
      recordDeployFailure(pool_address, { pool_name, error: error.message });
...
export async function getMyPositions({ force = false, silent = false, wallet_address = null } = {}) {
  ...
  const loadPositions = async () => { try {
    ...
    if (config.pnl.source === "rpc") {
      try {
        ...
        if (useLocalWallet) {
          ...
          reopenMisclosedPositions(rpcResult.positions);
          autoAdoptGhosts(rpcResult.positions);
    ...
    if (useLocalWallet) {
      ...
      reopenMisclosedPositions(positions);
      autoAdoptGhosts(positions);
...
// ─── Close Position ────────────────────────────────────────────
/**
 * Fetch a lightweight single-pool market snapshot (mcap/tvl/volume) from the
 * Meteora pool-discovery API. Used both for the at-close "exit market" record
 * and by the post-close price/volume tracker (post-close-tracking.js) so all
 * snapshots for a given pool come from the same consistent source.
 * Non-throwing — returns {} on any failure so callers can treat it as optional.
 */
export async function fetchPoolMarketSnapshot(poolAddress) {
  try {
    const { default: fetchImpl } = await import("node-fetch").catch(() => ({ default: globalThis.fetch }));
    ...
    const p = detail?.data?.[0];
    if (!p) return {};
    return {
      mcap: parseFloat(p?.token_x?.market_cap) || null,
      tvl: parseFloat(p?.tvl ?? p?.active_tvl) || null,
      volume: parseFloat(p?.volume) || null,
    };
  } catch {
    return {};
  }
...
/**
 * Best-effort RSI snapshot at close time — for the log line and record
 * only, never blocks or fails the close itself (the close transaction has
 * already succeeded by the time this runs). Reuses the same
 * interval/length knobs the RSI-confirm guards already use
 * (config.management.stopLossRsiInterval/stopLossRsiLength) rather than
 * introducing a separate setting just for this — "RSI(2)" here should
 * mean the same thing it means everywhere else in this codebase. Returns
 * {} (not {rsi: null, ...}) on any failure/missing mint, so recordClose's
 * extra.rsi != null check cleanly skips logging a misleading "n/a".
 */
async function snapshotRsiAtClose(baseMint) {
  if (!baseMint) return {};
  const rsiLengthAtClose = Number(config.management.stopLossRsiLength ?? 2);
  try {
    const payload = await fetchChartIndicatorsForMint(baseMint, {
      interval: config.management.stopLossRsiInterval || "15_MINUTE",
      rsiLength: rsiLengthAtClose,
    });
    const rsi = payload?.latest?.rsi?.value;
    return rsi != null ? { rsi, rsiLength: rsiLengthAtClose } : {};
  } catch {
    return {}; // non-fatal — the close already happened, this is just for the record
  }
}
...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    if (shouldUseLpAgentRelay()) {
      ...
      recordClose(position_address, reason || "agent decision", await snapshotRsiAtClose(livePosition?.base_mint || pool.lbPair.tokenXMint.toString()));
      ...
      if (tracked) {
        ...
        let exitMarket = {};
        try {
          const snap = await fetchPoolMarketSnapshot(poolAddress);
          if (snap.mcap != null || snap.tvl != null || snap.volume != null) {
            exitMarket = { exit_mcap: snap.mcap, exit_tvl: snap.tvl, exit_volume: snap.volume };
          }
        } catch { /* non-blocking */ }
        ...
        registerPostCloseWatch({
          position: position_address,
          pool: poolAddress,
          pool_name: tracked.pool_name || poolMeta.name || poolAddress.slice(0, 8),
          base_mint: closeBaseMint,
          close_reason: reason || "agent decision",
          close_pnl_pct: pnlPct,
          initial_value_usd: initialUsd,
          exit_mcap: exitMarket.exit_mcap,
          exit_tvl: exitMarket.exit_tvl,
          exit_volume: exitMarket.exit_volume,
        });

        // Permanent PnL-breakdown record — see pnl-history.js.
        appendPnlRecord({
          position: position_address,
          pool_name: tracked.pool_name || poolMeta.name || poolAddress.slice(0, 8),
          close_reason: reason || "agent decision",
          ...
          hold_minutes: tracked.deployed_at ? Math.round((Date.now() - new Date(tracked.deployed_at).getTime()) / 60000) : null,
        });
    ...
    if (hasLiquidity) {
      ...
      // Retries a blockhash-expiry ("has expired: block height exceeded")
      // by rebuilding via a fresh pool.removeLiquidity() call — the SDK
      // bakes in a fresh blockhash each time it's called, so simply
      // resending the SAME (already-expired) transaction object would
      // just fail the same way again. Confirmed happening in production
      // 2026-08 — every occurrence was at this exact step, always after
      // Step 1's claim had already completed, which is what makes this
      // retry safe: nothing from Step 2 itself has been sent yet
      // (closeTxHashes is still empty at this point), so a full rebuild
      // can't double-execute anything. Bounded at 2 retries — this is
      // meant to smooth over transient network congestion, not paper
      ...
      for (let attempt = 0; ; attempt++) {
        try {
          ...
          const expired = /block ?height exceeded|blockhash not found/i.test(e.message || "");
          if (!expired || attempt >= 2 || closeTxHashes.length > 0) throw e;
          log("close_warn", `Step 2: blockhash expired (attempt ${attempt + 1}/3) — rebuilding with a fresh blockhash and retrying`);
        }
      }
    } else {
    ...
    recordClose(position_address, reason || "agent decision", await snapshotRsiAtClose(pool.lbPair.tokenXMint.toString()));
