...
function buildPosition(f, prices, solUsd, meteora, solMode) {
  ...
  // Needed early for the cost-basis fallback below.
  ...
  // Meteora's /pnl deposit history can take minutes (occasionally hours) to
  // index a freshly-deployed position. Until it does, depositsUsd/depositsSol
  // read as 0, which used to blank out stop-loss/trailing-TP for the whole
  // window (observed up to ~3h in production logs 2026-07-22). Since a
  // not-yet-indexed position by definition has no withdrawals/claims yet
  // either, our own recorded deploy cost basis (state.js trackPosition ->
  // initial_value_usd) is a safe stand-in — it restores real protection
  // instead of running blind. Meteora's own numbers take over automatically
  // the moment they're indexed (this only fires while both deposit totals
  // read as 0).
  let costBasisFallback = false;
  if (depositsUsd <= 0 && depositsSol <= 0 && safeNum(tracked?.initial_value_usd) > 0) {
    depositsUsd = safeNum(tracked.initial_value_usd);
    depositsSol = solUsd > 0 ? depositsUsd / solUsd : 0;
    costBasisFallback = true;
  }
  ...
  // only when we couldn't price it at all. Guards against:
  //  - Jupiter outage → solUsd/priceX missing → balances collapse → false STOP_LOSS
  //  - missing Meteora deposits AND no deploy-time cost basis on file → 0 cost
  //    basis → garbage pnl / inflated value
  ...
  if (pnlPctSuspicious) {
    if (config.logging.suspiciousTickLogEnabled) {
  ...
  } else if (costBasisFallback) {
    log("pnl_warn", `${f.position.slice(0, 8)} deposits not yet indexed by Meteora — using deploy cost basis fallback ($${depositsUsd.toFixed(2)}) for PnL rules`);
  }
  ...
  return {
    ...
    pnl_pct_cost_basis_fallback: !!costBasisFallback,
