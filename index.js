...
import { pollPostCloseWatches } from "./post-close-tracking.js";
import { checkLiquidityCollapse, checkLiquidityRsiConfirm } from "./liquidity-guard.js";
import { checkStopLossRsiConfirm } from "./stoploss-rsi-guard.js";
import { checkExitRsiConfirm } from "./exit-rsi-guard.js";
import { checkPeakRsiExit } from "./peak-rsi-exit.js";
import { checkDeadPositions } from "./dead-position-guard.js";
import { checkFeeStall } from "./fee-stall-guard.js";
import { saveCandidateSnapshot } from "./candidate-snapshots.js";
import { startDiscoveryRecording, stopDiscoveryRecording } from "./tools/gmgn.js";
import { saveDiscoveryLog } from "./pool-discovery-log.js";
...
if (isMain) {
  ...
  try {
    ...
    log("startup", `Version: ${pkg.version} — bump this in package.json whenever a meaningful feature ships, so "which build is actually running" is one grep away instead of a log-forensics exercise.`);
  } catch (e) {
    log("startup_warn", `Could not read package.json version: ${e.message}`);
  }
  // Hardcoded, not derived from anything — the point is that this exact
  // line can only ever print if THIS exact version of index.js is what's
  // actually running. An older deployed build simply doesn't have this
  // line at all, which is the tell. Update this list whenever a guard/gate
  // ships that's meaningful enough to want to verify is actually live —
  // confirmed useful 2026-08: user-config.json had liquidityRsiConfirm
  // and suspiciousTickLogEnabled set, but the running process predated
  // both — took a full day of log forensics (comparing observed close
  // behavior against what those features should have logged) to catch,
  // instead of one grep for this line.
  const BUILD_FEATURES = [
    "stopLossRsiConfirm", "exitRsiConfirm(trailingTp/takeProfit/oor/lowYield)",
    "liquidityRsiConfirm", "peakRsiExit", "entryRsiGate", "rsiWaitLock",
    "suspiciousTickLogToggle", "triggerScreeningWhenIdle", "candidateSnapshots",
    "deadPositionGuard", "chartIndicatorRetry", "closeStepBlockhashRetry",
    "maxDownsideMode",
  ];
  log("startup", `Build features: ${BUILD_FEATURES.join(", ")}`);
  // Live cross-check: which of the ABOVE-CAPABLE features are actually
  // turned on in the config this process loaded. If a flag shows here as
  // "on" but its name isn't in the Build features line above, that's a
  // deployment-lag mismatch — config expects behavior this running code
  // doesn't have yet. If it shows as "off", nothing to check — that's
  // presumably deliberate.
  const configuredOn = [
    ["stopLossRsiConfirm", config.management.stopLossRsiConfirm],
    ["trailingTpRsiConfirm", config.management.trailingTpRsiConfirm],
    ["takeProfitRsiConfirm", config.management.takeProfitRsiConfirm],
    ["oorRsiConfirm", config.management.oorRsiConfirm],
    ["lowYieldRsiConfirm", config.management.lowYieldRsiConfirm],
    ["liquidityRsiConfirm", config.management.liquidityRsiConfirm],
    ["peakRsiExitEnabled", config.management.peakRsiExitEnabled],
    ["entryRsiGateEnabled", config.screening?.entryRsiGateEnabled],
    ["triggerScreeningWhenIdle", config.management.triggerScreeningWhenIdle],
    ["deadPositionGuardEnabled", config.management.deadPositionGuardEnabled],
    ["maxDownsideModeEnabled", config.strategy.maxDownsideModeEnabled],
  ].filter(([, on]) => on).map(([name]) => name);
  log("startup", `Config has these ON: ${configuredOn.length ? configuredOn.join(", ") : "(none of the above)"}`);
...
function stopCronJobs() {
  ...
  if (_cronTasks._postCloseTrackInterval) clearInterval(_cronTasks._postCloseTrackInterval);
  if (_cronTasks._liquidityGuardInterval) clearInterval(_cronTasks._liquidityGuardInterval);
  if (_cronTasks._stopLossRsiGuardInterval) clearInterval(_cronTasks._stopLossRsiGuardInterval);
  if (_cronTasks._exitRsiGuardInterval) clearInterval(_cronTasks._exitRsiGuardInterval);
  _cronTasks = [];
}
...
async function executeManagementActions(actionPositions, actionMap, { liveMessage = null, cur = "$" } = {}) {
  ...
  for (const p of actionPositions) {
    ...
    if (act.action === "CLOSE") {
      ...
      // signal/ruleName (set by the fast pnlPollInterval path or directly
      // on the closeRule object from the slow management cycle) rides
      // along so the lock gate can tell "this specific close never asked
      // to wait on anything of its own" apart from every other close
      // request — see executor.js's lock-check comment for why.
      const res = await executeTool("close_position", { position_address: p.position, reason }, { trusted: true, source: "deterministic-rule", signalType: act.signal ?? act.ruleName }).catch(e => ({ error: e.message }));
...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    if (positions.length === 0) {
      if (config.management.triggerScreeningWhenIdle) {
        log("cron", "No open positions — triggering screening cycle");
        mgmtReport = "No open positions. Triggering screening cycle.";
        runScreeningCycle().catch((e) => log("cron_error", `Triggered screening failed: ${e.message}`));
      } else {
        log("cron", "No open positions — triggerScreeningWhenIdle is off, screening stays on its own schedule");
        mgmtReport = "No open positions. Not triggering an extra screening cycle (triggerScreeningWhenIdle=false).";
      }
      return mgmtReport;
    }
    ...
    for (const p of positionData) {
      // Don't let an unpriceable tick (Jupiter outage, no cost basis at all)
      // feed a bogus value into peak tracking — that could arm a false
      // trailing-drop later. Cost-basis-fallback ticks are fine to feed in;
      // they're a real (if approximate) priced value, not suspicious.
      if (!p.pnl_pct_suspicious && confirmPeak(p.position, p.pnl_pct, 1)) {
        checkPeakRsiExit(p).catch((e) => log("cron_error", `Peak RSI exit check failed: ${e.message}`));
      }
    ...
    if (config.management.triggerScreeningWhenIdle && afterCount < config.risk.maxPositions && Date.now() - _screeningLastTriggered > screeningCooldownMs) {
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // Recording scoped tightly to just this call — every
    // fetchTopMeteoraDlmmPoolsForMint() invocation that happens during
    // candidate-building gets captured (currently only GMGN's Stage 3
    // triggers this — see tools/gmgn.js). Started/stopped right around
    // getTopCandidates so a later deploy_position call (after the LLM
    // decides) is NOT included — this is meant to be a screening-cycle
    // discovery log, not a catch-all for every pool lookup this process
    // ever makes.
    startDiscoveryRecording();
    const topCandidates = await getTopCandidates({ limit: 10 }).catch(() => null);
    const discoveryRecords = stopDiscoveryRecording();
    // Snapshot BEFORE any LLM reasoning runs — this is what was actually
    // available for the decision below, independent of whether the LLM
    // picks well, picks badly, or the cycle fails downstream for an
    // unrelated reason. See candidate-snapshots.js for why this exists.
    saveCandidateSnapshot(topCandidates, { cycle: "screening", limit: 10 });
    saveDiscoveryLog(discoveryRecords, { cycle: "screening" });
...
3. Call deploy_position (active_bin is pre-fetched above — no need to call get_active_bin).
   ${config.strategy.maxDownsideModeEnabled
     ? `maxDownsideModeEnabled is ON — bins_below/downside_pct you pass don't matter, the deploy always uses the maximum downside range the protocol allows with zero upside range. Just pass volatility for the record; no need to compute bins_below yourself.`
...
export function startCronJobs() {
  ...
  // node-cron expressions are wall-clock-anchored, not relative to process start
  // time -- "*/15 * * * *" always fires at :00/:15/:30/:45 regardless of when the
  // process started, so two instances started at different times still end up
  // triggering at the exact same minute. These offsets shift *this instance's*
  // trigger point -- set different values per instance's user-config.json to
  // stagger them (e.g. instance 1: minute offset 0, instance 2: minute offset 2).
  const offsetSec = Math.max(0, Math.min(59, Number(config.schedule.scheduleOffsetSec ?? 0)));
  // Minute offset uses cron's range/step syntax ("M-59/N" instead of "*/N") so
  // instances land on genuinely different minutes, not just different seconds
  // within the same minute -- e.g. offsetMin=2, interval=3 -> fires at :02, :05,
  // :08... instead of :00, :03, :06...
  const offsetMin = Math.max(0, Math.min(59, Number(config.schedule.scheduleOffsetMin ?? 0)));
  // NOT `${offsetMin}-59/${intervalMin}` — that range-step form only produces
  // multiple hits per hour when offsetMin + intervalMin <= 59. Past that (e.g.
  // scheduleOffsetMin=30 with a 30-minute interval: "30-59/30" only matches
  // minute 30 — 60 exceeds the 59 upper bound, so the second occurrence never
  // gets generated) it silently collapses to ONE run per hour regardless of
  // how short the configured interval is. Confirmed in production logs
  // 2026-08-01: screeningIntervalMin=30 was firing once/hour, always at :30 —
  // exactly this collapse. An explicit, modulo-wrapped minute list has no such
  // edge case: every minute in [0,59] congruent to offsetMin (mod intervalMin)
  // matches, regardless of how large offsetMin is relative to intervalMin.
  const everyMin = (intervalMin) => {
    const step = Math.max(1, Math.min(59, Number(intervalMin) || 1));
    const phase = offsetMin % step;
    const minutes = [];
    for (let m = phase; m < 60; m += step) minutes.push(m);
    return minutes.join(",");
  };

  const mgmtExpr = `${offsetSec} ${everyMin(config.schedule.managementIntervalMin)} * * * *`;
  const screenExpr = `${offsetSec} ${everyMin(config.schedule.screeningIntervalMin)} * * * *`;

  const mgmtTask = cron.schedule(mgmtExpr, async () => {
    if (_managementBusy) return;
    timers.managementLastRun = Date.now();
    await runManagementCycle();
  });

  const screenTask = cron.schedule(screenExpr, runScreeningCycle);

  log("cron", `Schedule: management="${mgmtExpr}" (every ${config.schedule.managementIntervalMin}m, offset ${offsetMin}m${offsetSec}s) | screening="${screenExpr}" (every ${config.schedule.screeningIntervalMin}m, offset ${offsetMin}m${offsetSec}s)`);

  const healthTask = cron.schedule(`${offsetSec} ${offsetMin} * * * *`, async () => {
    if (_managementBusy) return;
    _managementBusy = true;
...
Summarize the current portfolio health, total fees earned, and performance of all open positions. Recommend any high-level adjustments if needed.
  ...
  const briefingTask = cron.schedule(`${offsetSec} ${offsetMin} 1 * * *`, async () => {
    await runBriefing();
  }, { timezone: 'UTC' });

  // Every 6h — catch up if briefing was missed (agent restart, crash, etc.)
  const briefingWatchdog = cron.schedule(`${offsetSec} ${offsetMin} */6 * * *`, async () => {
  ...
  const pnlPollInterval = setInterval(async () => {
    ...
    try {
      ...
      for (const p of result.positions) {
        ...
        else if (closeRule) { signal = closeRule.ruleName ?? `RULE_${closeRule.rule}`; reason = closeRule.reason; rule = closeRule.rule; }
        ...
        _managementBusy = true;
        try {
          const actMap = new Map([[p.position, { action: "CLOSE", rule, reason, signal }]]);
  ...
  const postCloseTrackInterval = setInterval(() => {
    pollPostCloseWatches().catch((e) => log("cron_error", `Post-close tracking poll failed: ${e.message}`));
  }, 2 * 60 * 1000); // 2 min — this is passive research data, not decision-driving, so no need for the pnl poller's 3s cadence
  _cronTasks._postCloseTrackInterval = postCloseTrackInterval;

  const liquidityGuardMs = Math.max(1, Number(config.management.volumeTrendCheckIntervalMin ?? 7)) * 60 * 1000;
  const liquidityGuardInterval = setInterval(() => {
    checkLiquidityCollapse().catch((e) => log("cron_error", `Liquidity-collapse check failed: ${e.message}`));
  }, liquidityGuardMs);
  _cronTasks._liquidityGuardInterval = liquidityGuardInterval;

  // Shorter cadence than the liquidity guard on purpose — a position that's
  // hit stop-loss and is waiting for an RSI bounce needs to notice that
  // bounce promptly once it happens. The expensive part (the actual RSI
  // fetch) is throttled internally by stopLossRsiRecheckSec regardless of
  // how often this outer tick runs, so a 1-minute cadence here doesn't
  // multiply API load the way it would if there were no internal throttle.
  const stopLossRsiGuardMs = Math.max(1, Number(config.management.stopLossRsiGuardIntervalMin ?? 1)) * 60 * 1000;
  const stopLossRsiGuardInterval = setInterval(() => {
    checkStopLossRsiConfirm().catch((e) => log("cron_error", `Stop-loss RSI-confirm check failed: ${e.message}`));
  }, stopLossRsiGuardMs);
  _cronTasks._stopLossRsiGuardInterval = stopLossRsiGuardInterval;

  // Same cadence reasoning as the stop-loss guard above — cheap outer
  // tick, actual RSI fetches internally throttled by exitRsiRecheckSec.
  const exitRsiGuardMs = Math.max(1, Number(config.management.exitRsiGuardIntervalMin ?? 1)) * 60 * 1000;
  const exitRsiGuardInterval = setInterval(() => {
    checkExitRsiConfirm().catch((e) => log("cron_error", `Exit RSI-confirm check failed: ${e.message}`));
    checkLiquidityRsiConfirm().catch((e) => log("cron_error", `Liquidity RSI-confirm check failed: ${e.message}`));
    checkDeadPositions().catch((e) => log("cron_error", `Dead-position check failed: ${e.message}`));
    checkFeeStall().catch((e) => log("cron_error", `Fee-stall check failed: ${e.message}`));
  }, exitRsiGuardMs);
  _cronTasks._exitRsiGuardInterval = exitRsiGuardInterval;
...
function getDeterministicCloseRule(position, managementConfig) {
  ...
  // rule entirely (see config.js's comment) — same switch state.js's
  // fast-poll path checks, kept in sync so both paths agree.
  // Stands down when stopLossRsiConfirm is on — stoploss-rsi-guard.js owns
  // this decision instead (see state.js's matching bypass for the same reason).
  const stopLossEnabled = managementConfig.stopLossEnabled !== false;
  if (stopLossEnabled && !managementConfig.stopLossRsiConfirm && !pnlSuspect && position.pnl_pct != null && position.pnl_pct <= managementConfig.stopLossPct) {
    return { action: "CLOSE", rule: 1, ruleName: "STOP_LOSS", reason: "stop loss" };
  }
  if (!managementConfig.takeProfitRsiConfirm && !pnlSuspect && position.pnl_pct != null && position.pnl_pct >= managementConfig.takeProfitPct) {
    return { action: "CLOSE", rule: 2, ruleName: "TAKE_PROFIT", reason: "take profit" };
  }
  if (
    position.active_bin != null &&
    position.upper_bin != null &&
    managementConfig.outOfRangeBinsToClose != null &&
    // Extreme-safety-valve only: the primary exit for a pump is the wider
    // pump-trailing-stop in state.js's updatePnlAndCheckExits (checked before
    // this function ever runs, since it's evaluated first in both the fast
    // poller and the management cycle). This rule is now a last-resort
    // circuit breaker for a move so extreme it's a thin-liquidity/manipulation
    // risk regardless of trend -- outOfRangeBinsToClose no longer closes the
    // position on its own; it only sets pumpedFarAboveRange (which widens the
    // trailing-drop threshold). See CLAUDE.md / config for pumpSafetyMultiplier.
    position.active_bin > position.upper_bin + managementConfig.outOfRangeBinsToClose * (managementConfig.pumpSafetyMultiplier ?? 4)
  ) {
    return { action: "CLOSE", rule: 3, ruleName: "PUMPED_FAR_ABOVE_RANGE", reason: "pumped extremely far above range — safety valve" };
  }
  if (
    !managementConfig.oorRsiConfirm &&
    position.active_bin != null &&
    position.upper_bin != null &&
    position.active_bin > position.upper_bin &&
    (position.minutes_out_of_range ?? 0) >= managementConfig.outOfRangeWaitMinutes
  ) {
    return { action: "CLOSE", rule: 4, ruleName: "OUT_OF_RANGE", reason: "OOR" };
  }
  if (
    !managementConfig.lowYieldRsiConfirm &&
    position.fee_per_tvl_24h != null &&
    position.fee_per_tvl_24h < managementConfig.minFeePerTvl24h &&
    (position.age_minutes ?? 0) >= (managementConfig.minAgeBeforeYieldCheck ?? 60)
  ) {
    return { action: "CLOSE", rule: 5, ruleName: "LOW_YIELD", reason: "low yield" };
  }
  return null;
}
