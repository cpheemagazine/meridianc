...
/**
 * Attach a finalized post-close tracking summary (see post-close-tracking.js)
 * onto the matching deploy record, found by position address. No-op if the
 * pool or the specific deploy can't be found (e.g. pool-memory.json was
 * pruned/reset between close and finalization).
 */
export function attachPostCloseOutcome(poolAddress, position, outcome) {
  if (!poolAddress || !position || !outcome) return false;
  const db = load();
  const entry = db[poolAddress];
  if (!entry) return false;
  const deploy = entry.deploys.find((d) => d.position === position);
  if (!deploy) return false;
  deploy.post_close_outcome = outcome;
  save(db);
  log("pool-memory", `Attached post-close outcome to ${entry.name} (${position.slice(0, 8)}): ${outcome.verdict}`);
  return true;
}

/**
 * Cool a pool down after a failed deploy attempt. Deploy failures (bad
 * simulation, wrong quote token, insufficient funds, transient RPC issues,
 * etc.) previously had no memory at all — a structurally broken pool would
 * get re-offered to the LLM and re-attempted every single screening cycle.
 * Confirmed live: one pool got 10 failed deploy attempts over 3.5 hours
 * before the underlying cause (non-SOL quote token) was fixed at the
 * screening filter level. This is the safety net for whatever that filter
 * doesn't catch — any deploy failure, any cause.
 *
 * Short cooldown by design (a few hours, not the multi-day pool-blacklist
 * cooldowns used for repeated bad *closes*) — most failure causes are
 * pool-specific and permanent (wrong quote token) but some could be
 * transient (RPC hiccup, momentary balance issue), so this isn't a
 * permanent blacklist.
 */
export function recordDeployFailure(poolAddress, { pool_name, base_mint, error, hours = 6 } = {}) {
  ...
  const entry = db[poolAddress];
  entry.deploy_failures = (entry.deploy_failures || 0) + 1;
  const reason = `deploy failed x${entry.deploy_failures}: ${String(error || "unknown error").slice(0, 200)}`;
  // Escalate: a single failure could be transient, so start short; repeated
  // failures against the same pool are almost certainly a permanent
  // incompatibility (like a non-SOL quote token) — cool it down harder.
  const cooldownHours = entry.deploy_failures >= 3 ? hours * 4 : hours;
  const cooldownUntil = setPoolCooldown(entry, cooldownHours, reason);
  save(db);
  log("pool-memory", `Deploy-failure cooldown on ${entry.name} (attempt #${entry.deploy_failures}): ${cooldownHours}h until ${cooldownUntil}`);
  return cooldownUntil;
}
...
export function recallForPool(poolAddress) {
  ...
  // Most recent post-close outcome (see post-close-tracking.js) — did the exit
  // leave profit on the table, or was it the right call?
  const withOutcome = [...entry.deploys].reverse().find((d) => d.post_close_outcome);
  if (withOutcome) {
    const o = withOutcome.post_close_outcome;
    lines.push(`POST-CLOSE CHECK [${withOutcome.close_reason || "close"}]: ${o.summary}`);
  }
